...
- **게임이 닫혀 있는 동안** — 숨은 미니게임을 아무도 못 찾고 지나가서,
  후광을 뿜고 펄쩍이며 `여기 5번 눌러주세요!` 라고 계속 조른다. 아래 [숨은 게임 알리기](#숨은-게임-알리기) 참고.
...
금갱이를 연속 5번 두드리면 미니게임 **금갱런**이 열린다. 그런데 아무도 이게 있는 줄
몰라서, **게임이 닫혀 있는 동안에는 금갱이가 계속 조른다.**

- 말풍선의 기본 상태가 "저 좀 5번 두드려 주세요… 네? 🙏" 같은 문구다. 4.6초마다 갈아 끼우고,
  일하는 중·대기 중 메시지가 뜰 때만 잠깐 양보한다.
- 스프라이트 뒤로 후광 두 겹이 엇갈려 퍼지고, 대기 동작도 평소보다 크게 펄쩍인다.
- 스프라이트 위에 `여기 5번 눌러주세요!` 배지와 **5칸 진행 표시**가 붙는다. 한 번만 눌러 봐도
  `4번 더!` 와 점 1칸이 차올라서, 몇 번 더 눌러야 하는지가 바로 보인다. 배지를 눌러도 카운트된다.
- 게임이 열리면(`buddyState.gameOpen`) 이 연출은 전부 멈춘다.

연타 판정도 같이 손봤다. 전에는 "2.5초 안에 5번"이라 화면에 대놓고 안내하기엔 너무 빡빡했다.
지금은 **클릭 사이 간격이 1.5초를 넘으면** 처음부터 다시 센다(`GAME_CLICK_GAP`).
천천히 눌러도 손만 멈추지 않으면 열린다.

`prefers-reduced-motion` 에서는 후광을 감추고 애니메이션을 끄되, **안내 배지와 문구는 남긴다**
(움직임이 아니라 정보이기 때문). 화면 낭독기에는 배지 대신 버튼의 `aria-label` 로 같은 내용을 전한다.

### 명예의 전당 (금갱런 랭킹)

기록은 브라우저가 아니라 **서버(`run_scores`)** 에 남아서 모든 방문자가 같은 순위를 본다.

- **사람(uid)당 한 줄**만 두고 자기 최고 기록만 남긴다. 한 사람이 1·2·3등을 독식하면
  시상대가 의미가 없기 때문. 자기 기록을 넘겼을 때만 갱신된다(`saveRunScore`).
- 게임이 끝나면 이름을 넣어 등록하고, 응답으로 받은 등수를 `지금 3위예요` 처럼 보여준다.
  이름은 `localStorage` 에 기억해 두고, 이 브라우저에 기록이 없으면 서버에 저장된 이름을 채운다.
- 1·2·3등은 시상대에 세운다. 금·은·동 메달을 건 금갱이가 환호하듯 뛰고, 메달과 단상에
  광택이 훑고 지나가며 주변에 별이 반짝인다. 등수별로 달라지는 건 `--medal-a/-b/-ink` 와
  단상 높이뿐이고 구조는 같다. 4위부터는 줄 목록이다.
- 동점이면 **먼저 세운 기록**이 위로 간다.

> 이름 입력칸이 생기면서 스페이스바가 점프로 먹히던 문제도 같이 고쳤다
> (`onKeydown` 에서 `input`/`textarea` 포커스를 먼저 걸러낸다).

| 메서드 | 경로 | 하는 일 |
| --- | --- | --- |
| GET | `/api/geumgaengrun/scores` | 상위 10명 + 내 기록 |
| POST | `/api/geumgaengrun/scores` | `{ name, score }` 등록 → 갱신 여부·내 등수·갱신된 순위표 |
...
| GET·POST | `/api/geumgaengrun/scores` | 금갱런 명예의 전당 조회·기록 등록 |

- 테이블: `posts` / `comments` / `post_likes` / `visits` / `visit_log` / `profiles` /
  `run_scores` / `meta`.
